...
class DictionaryCollect:
    ...
    def Collect(self, **kwargs):
        ...
        if return_type == 'Repository':            
            ...
            if not DataExistReturnValue:
                DataDeleteTargetTableList = ['스키마정보', '테이블정보', '컬럼정보']
            ...
            ### 데이터 적재
            SchemaInsertSQL = """INSERT INTO 스키마정보 (DB번호, 스키마명)
                                 VALUES (?, ?)"""
            TableInsertSQL = """INSERT INTO 테이블정보 (DB번호, 스키마명, 테이블명, 테이블코멘트, 테이블사이즈)
                                 VALUES (?, ?, ?, ?, ?)"""    
            ColumnInsertSQL = """INSERT INTO 컬럼정보 (DB번호, 스키마명, 테이블명, 컬럼명, 컬럼코멘트, 
    ...
    def RepositoryDataExistCheck(self, DB번호):
        ...
        sql = """SELECT SUM(CNT) 
                   FROM  (SELECT COUNT(*) CNT FROM 스키마정보 WHERE DB번호 = :DB번호 AND ROWNUM = 1
                            UNION ALL
                          SELECT COUNT(*) FROM 테이블정보 WHERE DB번호 = :DB번호 AND ROWNUM = 1
                            UNION ALL
                          SELECT COUNT(*) FROM 컬럼정보 WHERE DB번호 = :DB번호 AND ROWNUM = 1)"""
